...
/* login temporal quitar cuando el sitema este en produccion
	...
	*/
	if(isset($_REQUEST['noentrar']))
	$loginVerifica='f';
	else $loginVerifica='t';
...
<?php
	...
	if (isset($_REQUEST['logout'])) {
		if(loginTemp($_COOKIE['user_id'])=='f') echo $loginVerifica=='f';
		elseif(loginTemp($_COOKIE['user_id'])=='t') echo $loginVerifica=='t';
		...
		$redirect = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'].'?noentrar';
		header('Location: ' . filter_var($redirect, FILTER_SANITIZE_URL));
		return;
	}
...
<?php
	...
	//	$user = $abcis->get_row("select count(id_card) as n, id_card, nickname from abc_view_staff where password = md5('{$_POST['txtpassword']}') and login = '{$_POST['txtusername']}' group by id_card, nickname;");
		...
		if( !empty($user->id_card) and $user->n == 1 and loginTemp($user->id_card)=='t' ):
		//if( !empty($user->id_card) and $user->n == 1 ):
			...
			header( "Location: $website/index.php" ) ;
		elseif(loginTemp($user->id_card)=='f'):
			$loginVerifica='f';
		else:
...
<div class="span12">
<div class="jumbotron ">
   <h1>Welcome at Booking System </h1>
   <blockquote>
   <p class="lead">The system will be available soon </p>
   <small>Thanks for your understanding  <cite title="Source Title">ICT Support</cite></small>
  </blockquote>
</div>
</div>
