delimiter $$
create function checkupdate(eventid int, resourceid int, qu int, bookingdate date, stime time, etime time) returns int
begin
    ...
    ) and resource_id = resourceid and id != eventid;
    set j = 0;
    select quantity into j from resource where id = resourceid;
    set available = j - i;
